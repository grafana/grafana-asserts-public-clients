...
> PatternDto GetCustomByName(ctx, name).XScopeOrgID(xScopeOrgID).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.CustomRcaPatternControllerAPI.GetCustomByName(context.Background(), name).XScopeOrgID(xScopeOrgID).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CustomRcaPatternControllerAPI.GetCustomByName``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetCustomByName`: PatternDto
	fmt.Fprintf(os.Stdout, "Response from `CustomRcaPatternControllerAPI.GetCustomByName`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetCustomByNameRequest struct via the builder pattern
