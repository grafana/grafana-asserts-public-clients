...
> ConnectedEntitiesResponseDto LookupConnectedEntities(ctx, namespace).Type_(type_).Name(name).Domain(domain).ConnectedType(connectedType).Limit(limit).Offset(offset).Scope(scope).XScopeOrgID(xScopeOrgID).Execute()
...
func main() {
	...
	connectedType := "connectedType_example" // string | Restrict results to connected entities of this type (optional)
	limit := int32(56) // int32 |  (optional) (default to 25)
	offset := int32(56) // int32 |  (optional) (default to 0)
	scope := map[string]string{"key": map[string]string{"key": "Inner_example"}} // map[string]string | Exact scope key/value pairs identifying the anchor entity (scope[key]=value). (optional)
	xScopeOrgID := "2944" // string | Grafana Tenant/Stack ID (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.KnowledgeGraphReadAPIAPI.LookupConnectedEntities(context.Background(), namespace).Type_(type_).Name(name).Domain(domain).ConnectedType(connectedType).Limit(limit).Offset(offset).Scope(scope).XScopeOrgID(xScopeOrgID).Execute()
	if err != nil {
 ...
 **name** | **string** |  | 
 **domain** | **string** |  | 
 **connectedType** | **string** | Restrict results to connected entities of this type | 
 **limit** | **int32** |  | [default to 25]
 **offset** | **int32** |  | [default to 0]
 **scope** | **map[string]map[string]string** | Exact scope key/value pairs identifying the anchor entity (scope[key]&#x3D;value). | 
...
> EntityReadResponseDto LookupEntity(ctx, namespace).Type_(type_).Name(name).Domain(domain).Scope(scope).XScopeOrgID(xScopeOrgID).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.KnowledgeGraphReadAPIAPI.LookupEntity(context.Background(), namespace).Type_(type_).Name(name).Domain(domain).Scope(scope).XScopeOrgID(xScopeOrgID).Execute()
	if err != nil {
